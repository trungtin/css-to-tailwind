...
test('getVariantFromSelector should handle multiple rule with variant', () => {
    const [{ variants: variantsFoo }, { variants: variantsBar }] = getVariantFromSelector({
        selector: '.foo:focus, .bar:active:hover',
        mediaRuleValue: null,
        responsiveBreakpointNames,
        roundResponsiveViewBreakpoints,
    });
    expect(variantsFoo).toEqual(['focus']);
    expect(variantsBar).toEqual(['active', 'hover']);
});
