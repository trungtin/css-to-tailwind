...
function filterTailwind(inputNormalized, selector) {
    ...
    // TODO: merge and remove redundant classes
    // TODO: for e.g mb-4, mt-4 -> my-4, use hint from tailwind plugins list
...
function transformRule(variant, inputNormalized) {
    return Object.keys(inputNormalized).map((selector) => {
        ...
        const variantPrefix = variant === 'base' ? '' : variant.split(',').join(':') + ':'

        const resultArray = resultTailwind;

        const tailwind = resultArray.map(s => variantPrefix + s).join(' ');
...
async function transform(inputCss) {
    ...
    const inputNormalized = normalizer(await parseInputCss(inputCss), options);
